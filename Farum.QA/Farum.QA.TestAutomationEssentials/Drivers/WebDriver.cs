...
namespace Farum.QA.TestAutomationEssentials.Drivers
{
    ...
    public class WebDriver : IDisposable
    {
        ...
        public String GetScreenshot(String screenshotPrefix)
        {
            try
            {
                if(!Directory.Exists(Path.GetFullPath(@"..\TestResults\").ToString() + "Screenshots\\"))
                {
                    if (_configurationDriver.DebugMode)
                    {
                        _specFlowOutputHelper.WriteLine("Screenshot path missing. Creating one...");
                    }
                    Directory.CreateDirectory(Path.GetFullPath(@"..\TestResults\Screenshots").ToString());
                }
                var screenshotPath = Path.GetFullPath(@"..\TestResults\Screenshots").ToString();
